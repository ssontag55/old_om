...
for line in inlines:
	...
	if(startatLine<14):
		...
		print valuelist[0]
		#print "insert into data.data_values(collection_date,station_id,sensor_id,value20) values ('"+valuelist[0]+"',"+str(stationID)+","+str(sensorID)+str(1000*float(str(valuelist[1]).replace('\n','')))+");"
		#exit();
		try:
			curs.execute("insert into data.data_values(collection_date,station_id,sensor_id,value20) values ('"+valuelist[0]+"',"+str(stationID)+","+str(sensorID)+","+str(valuelist[1]).replace('\n','')+");")
			totalRecords+=1
		except:
