...
# Main variables
sensorDict ={"ctd":1,"adcp":2,"spectra":3,"awac":4,"tide_gauge":5,"aqd":6,"weather":7,"waver":8}
...
for line in inlines:
	...
	if(startatLine<16):
		...
		print valuelist[0]
		#print "insert into data.data_values(collection_date,station_id,sensor_id,value21,value20) values ('"+valuelist[0]+"',"+str(stationID)+","+str(sensorID)+","+str(valuelist[1])+","+str(valuelist[2]).replace('\n','')+");"
		#exit();
		try:
			curs.execute("insert into data.data_values(collection_date,station_id,sensor_id,value24,value25,value26) values ('"+valuelist[0]+"',"+str(stationID)+","+str(sensorID)+","+str(valuelist[1])+","+str(valuelist[2])+","+str(valuelist[3]).replace('\n','')+");")
			totalRecords+=1
		except:
