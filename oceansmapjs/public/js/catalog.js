var CatalogView = Backbone.View.extend({
  ...
  refreshGraphs : function() {
    var that = this;
    var h = that.model.get('graph0');
    if (h) {
      h.graphId = 0;
      $.when(catalogView.goGraph(h)).done(function() {
        var h = that.model.get('graph1');
        if (h) {
          h.graphId = 1;
          catalogView.goGraph(h);
        }
  ...
  goGraph : function(info,isNew) {
    ...
    if (info.param.indexOf('current') >= 0) {
        ...
        var content = '<table id=\'depth-options\' class=\'ui table segment\'><tr>' + tr.join('</tr><tr>') + '</tr></table><div data-param=\'' + info.param + '\' class=\'show-graph small ui button\' data-sid=\'' + info.sid + '\' data-start=\'' + info.station_dates[0] + '\' data-end=\'' + info.station_dates[1] + '\'>Show graph</div>';
        ...
        $('#param0Stats .button').popup({
          ...
          ,onCreate : function() {
            ...
            $('.show-graph').click(function() {
              ...
              var station_dates = [$(this).data('start'),$(this).data('end')];
              var w             = $('#big_graph').width() - 15
              var h             = $('#catalog').height() - 120
              var subsample     = _.isUndefined(catalogView.model.get('graph0').subsample) || catalogView.model.get('graph0').subsample;
              var checked       = [];
              ...
              return $.ajax({
                ...
                ,subsample     : subsample
                ,url           : 'http://map.asascience.com/EGDataViewer/Scripts/proxy.php?http://10.90.209.21:8080/oceansmap65/metobs/gettimeseriescurrentsimage/?st=' + sid + '&starttime=' + dates.values.min.format('yyyy-mm-dd"T"HH:00:00') + '&endtime=' + dates.values.max.format('yyyy-mm-dd"T"HH:00:00') + '&y=surface&d=' + checked.join(',') + '&attr=and&h=' + h + '&w=' + w + '&samp=' + subsample
                ,success  : function(r) {
                  ...
                  if (r == 'Please check your request') {
                    ...
                    $('#param0Controls').html('<div class="ui hover tiny button clear-graph" data-graph-id="' + this.i + '"><i class="close icon"></i>Clear</div> <div class="ui hover tiny button zoom-to-dates" data-start="' + this.station_dates[0] + '" data-end="' + this.station_dates[1] + '"><i class="icon calendar"></i> View all times</div> <div class="ui toggle checkbox subsample"><input ' + (this.subsample ? 'checked' : '') + ' type="checkbox" name="' + this.i + '"><label>Sub-sampled</label></div>');
                  }
                  $('#param0Controls .ui.toggle.checkbox').checkbox('setting','onChange',function() {
                    var h = catalogView.model.get('graph0');
                    h.subsample = $(this).prop('checked');
                    h.graphId = '0';
                    catalogView.goGraph(h);
                  });
      ...
      return $.ajax({
        ...
        ,subsample     : _.isUndefined(info.subsample) || info.subsample
        ,dateRange     : dates.values
        ,url           : 'http://map.asascience.com/EGDataViewer/Scripts/proxy.php?http://10.90.209.218080/oceansmap65/metobs/gettimeseries/?attr=and&param=' + info.param + '&st=' + info.sid + '&starttime=' + dates.values.min.format('yyyy-mm-dd"T"HH:00:00')+ '&endtime=' + dates.values.max.format('yyyy-mm-dd"T"HH:00:00') + '&y=adf' + '&samp=' + (_.isUndefined(info.subsample) || info.subsample)
            ...
            $('#param' + pIdx + 'Controls').html('<div class="ui hover tiny button clear-graph" data-graph-id="' + pIdx + '"><i class="close icon"></i>Clear</div> <div class="red ui hover tiny button zoom-to-dates" data-start="' + station_dates[0] + '" data-end="' + station_dates[1] + '"><i class="icon calendar"></i> View all times</div>');
              ...
              var chartOptions = {
                ...
                ,hAxis            : {
                  ...
                  ,viewWindow    : {min   : dateRange.min,max : dateRange.max}
                }
                ,tooltip          : {isHtml : true} 
              ...
              var help = "<div class='subsample-help'><i class='help icon link' data-html='" + 'Instantaneous data on the hour.' + "'></i></div>";
              ...
              $('#param' + pIdx + 'Controls').html('<div class="ui hover tiny button clear-graph" data-graph-id="' + pIdx + '"><i class="close icon"></i>Clear</div> <div class="ui hover tiny button zoom-to-dates" data-start="' + station_dates[0] + '" data-end="' + station_dates[1] + '"><i class="icon calendar"></i>View all times</div> ' + help + ' <div class="ui toggle checkbox subsample"><input ' + (subsample ? 'checked' : '') + ' type="checkbox" name="' + pIdx + '"><label>Sub-sampled</label></div>');
              $('#param' + pIdx + 'Controls .help').popup({position:'bottom left'});
              $('#param' + pIdx + 'Controls .ui.toggle.checkbox').checkbox('setting','onChange',function() {
                var h = catalogView.model.get('graph' + $(this).attr('name'));
                h.subsample = $(this).prop('checked');
                h.graphId = $(this).attr('name');
                catalogView.goGraph(h);
              });
              $('#catalog').spin(false);
            });
  ...
  drawChart : function() {
    if (catalogView.model.get('chart')) {
      ...
      if (!_.isEmpty(chartData)) {
        ...
        var uom = catalogView.model.get('chart0Data').getColumnLabel(1).substr(catalogView.model.get('chart0Data').getColumnLabel(1).lastIndexOf('(') + 1);
        chartOptions.vAxes = [{title : uom.substr(0,uom.length - 1)}];
        if (chartData.length > 1) {
          ...
          var uom = catalogView.model.get('chart1Data').getColumnLabel(1).substr(catalogView.model.get('chart1Data').getColumnLabel(1).lastIndexOf('(') + 1);
          chartOptions.vAxes.push({title : uom.substr(0,uom.length - 1)});
        }
        else {
