var MapView = Backbone.View.extend({
  initialize: function() {
    ...
    // CHANGEME.  Modified leaflet_min.css (can't believe I did that) to change stack order of markers vs. overlays.
    // This way the current vectors are drawn underneath the station dots.
    var currents = L.featureGroup();
    ...
    this.timesliderdate = new Date(2012,00,11).format('yyyy-mm-dd"T"HH:MM:ss');

    this.model.set('selectedType', 'all');
    this.model.set('selectedDeployment', '1');
    this.model.set('currents',currents);
  },
...
*/
  ...
  changeDeplmnt: function(evt) {
    ...
    for(var marker in mapView.model.stationlayers['Stations']._layers)
    {
      // CHANGEME: Deployment 8 = offshore wind which needs to be seen all the time.
      var visibl = mapView.model.stationlayers['Stations']._layers[marker].feature.properties['deploy'] == 8 ? 1 : 0;
      if(evt == 'all') {
        visibl = 1;
...
*/
  ...
  popUp: function(f,l) {
    var out = []
      , popup = L.popup({ minWidth: 330,className : 'alwaysontop' })
  ...
  initializeoverlays: function() {
    ...
    function bindlistele(html, layer) {
     $modelslist
      ...
      .children().last() // select new element to bind to
        .checkbox({
          onEnable: function() {
            // force the infernal thing to refresh
            that.model.layers[layer].setParams({'___d' : new Date().getTime()});
            that.map.addLayer(that.model.layers[layer]);
            that.model.layers[layer].redraw();
            // this seems to be the only way to get the stations to be on TOP of the layer
            that.model.layers[layer].bringToBack();
            mapView.trigger('timesliderChange',new Date($('#timeslidermap').slider('value')).format('UTC:yyyy-mm-dd"T"HH:MM:ss'));
          },
          onDisable: function() {
    ...
    for (i=0; i<overlays.length; i++) {
     ...
     if (/^SCENARIO/.test(overlays[i].title)) {
       // CHANGEME!  Hard-wiring a time and bbox so that the scenario will have a legend.  These should be dynamic.
       popuphtml =  '<p style="text-align:center;"><p style="text-align:center;"><img src="http://mapappdev.asascience.com/oilmapwebservice20/drawEDS.aspx?&zlayer=1&res_id=100026&stride=AUTO%2C20&xycolor=true&grid=false&session_id=35_zxnr97182ZX970&user_id=35&Contour=false&layers='+ overlays[i].name+'&transparent=true&styles=&request=GetLegendGraphic&width=112&version=1.1.1&format=image/png&height=155&TIME=2005-01-05T09:00:00&bbox=4317163.357546754,-1395434.388374178,4911537.689492284,-974724.9846925685"></p>';
       this.model.layers[overlays[i].title] = L.nonTiledLayer.wms('http://mapappdev.asascience.com/oilmapwebservice20/drawEDS.aspx?&zlayer=1&res_id=100026&stride=AUTO%2C20&xycolor=true&grid=false&session_id=35_zxnr97182ZX970&user_id=35&Contour=false&', {
         layers: overlays[i].name,
         format: 'image/png',
         //add three for the list of basemap layers
         // zIndex: overlays.length + i+3
       });
       this.model.layers[overlays[i].title].updateTime = function(t) {
         var d = Date.parse(t);
         d.setFullYear(2005);
         var e = L.Util.formatTimeWMS(d.format('UTC:yyyy-mm-dd"T"HH:MM:ss'));
         this.wmsParams.___d = new Date().getTime();
         this.wmsParams.time = e;
         this._update();
       }
     }
     else {
       ...
       html = "<div class='item' style='padding-bottom: 6px;' id='" + overlays[i].title + "-popup'>"
           +   "<div class='ui checkbox toggle' style='margin-left: -30px;'>"
           +     "<input type='checkbox' />"
           +     "<label style='font-size:14px;'>" + overlays[i].title + "</label>"
           +   "</div>"
           +   "<i class='help icon link' style='position: relative; right: -225px; margin-top: -3px;' "
  ...
  initializeTimeSliderMap: function (daterange){
    ...
    if(daterange == 'new'){
        ...
        this.timesliderdate = new Date().format('yyyy-mm-dd"T"HH:MM:ss');
        ...
        $timesliderdetail.html(backtime.toLocaleString().slice(0,-6) + ' ' + backtime.toLocaleString().substr(-2));
        this.timesliderdate = time.format('yyyy-mm-dd"T"HH:MM:ss');
    }
    ...
    $('#timeslidermap').slider({
      ...
      stop: function (event,ui){
        ...
        mapView.timesliderdate = new Date(unixtime).format('yyyy-mm-dd"T"HH:MM:ss');
        mapView.trigger('timesliderChange', new Date(unixtime).format('UTC:yyyy-mm-dd"T"HH:MM:ss'));
      }
    });
  ...
  initializerealtime: function() {
    ...
    function bindlistele(layer, popuphtml, enablecheckbox) {
      ...
      html =  "<div class='ui checkbox toggle'>"
           +     "<input type='checkbox' />"
           +     "<label style='font-size:14px;'>Show Surface Currents</label>"
           +   "</div>"
      ...
      $checkbox
        .checkbox({
          onChange: function() {
            mapView.syncMarkers(mapView.model.get('markerData'));
          }
        })
      ;
    }
    ...
    //add anadarko stations
    $.ajax({
    url: 'http://map.asascience.com/EGDataViewer/Scripts/proxy.php?http://172.16.16.125:8080/oceansmap65/metobs/getstations/?attr=and&mode=true&y=as',
    //localhost
      ...
      $checkbox
        .checkbox({
          onEnable: function() {
            ...
            mapView.model.gislayers[layer].refresh();
          },
          onDisable: function() {
  ...
  syncStationList : function() {
    ...
    // CHANGEME! Deployment 8 again.
    var t0 = Date.parse(_.pluck(_.filter(stations,function(o){return o.deploy != 8}),'start').sort().shift());
    t0.setMinutes(0);
    t0.setMilliseconds(0);
    t0.setHours(0);
    // CHANGEME! Deployment 8 again.
    var t1 = Date.parse(_.pluck(_.filter(stations,function(o){return o.deploy != 8}),'end').sort().pop());
    t1.setMinutes(0);
    ...
    $('#stationListMain .graphA').click(function() {
      ...
      if (/current/i.test($(this).parent().data('param'))) {
        if ($('#stationListMain .graphB.inverted').length > 0) {
          ...
          $('#modal-alert').modal('show').modal('setting','onHide',function() {
            // Semantic having a tough time allowing allowMultiple to work as advertised, so do it manually here.
            setTimeout(function(){$('#param0Stats .button').popup('show')},500);
          });
          catalogView.model.unset('chart1Data');
          catalogView.model.unset('graph1');
  ...
  syncMarkers : function(jsony) {
    var currents = mapView.model.get('currents');
    currents.clearLayers();
    ...
    for (var marker in mapView.model.stationlayers['Stations']._layers) {
      mapView.model.stationlayers['Stations']._layers[marker].hideLabel();
      mapView.model.stationlayers['Stations']._layers[marker].label.setContent(
        mapView.model.stationlayers['Stations']._layers[marker].feature.properties['station_name']
      );
    }

    // show any surface currents (doesn't matter if station is collapsed/expanded)
    $('#stationListMain a').each(function() {
      // get marker and data mapping
      var sname = $(this).text();
      var layer = _.find(mapView.model.stationlayers['Stations']._layers,function(o){return o.feature.properties['station_name'] == sname});
      var data  = _.findWhere(jsony.values,{sid : layer.feature.properties.sid});
      if (data && !_.isUndefined(data['Current Direction']) && $('#stations .checkbox input').prop('checked')) {
        L.marker([layer._latlng.lat,layer._latlng.lng],{icon : L.icon({
           iconUrl  : 'http://mymaracoos.org/fish/icon.php?size=115,115&dir=' + data['Current Direction'] + '&arrow&noCircle&label&mag=' + Math.round(data['Current Speed'])
          ,iconSize : [74,75]
        })}).addTo(currents);
      }
    });

    // show popup info for any expanded stations
    $('#stationListMain .title.active a').each(function() {
      // get marker and data mapping 
      var sname = $(this).text();
      var layer = _.find(mapView.model.stationlayers['Stations']._layers,function(o){return o.feature.properties['station_name'] == sname});
      var data  = _.findWhere(jsony.values,{sid : layer.feature.properties.sid});
      if (data) {
        layer.label.setContent(mapView.parseObjToStr(data,sname));
      }
      else {
        layer.label.setContent(sname + '<br>' + 'no data');
      }
      layer.showLabel();
    });
  },
  ...
  goParam : function(graphId,sid,station_name,depth_values,param,station_dates,group_dates,activeAlert) {
